package com.tadpole.cloud.platformSettlement.modular.inventory.service;

import cn.stylefeng.guns.cloud.model.web.response.ResponseData;
import com.tadpole.cloud.platformSettlement.api.inventory.entity.ErpWarehouseCode;
import com.tadpole.cloud.platformSettlement.api.inventory.model.params.ErpWarehouseCodeParam;
import com.baomidou.mybatisplus.extension.service.IService;
import java.util.List;
import java.util.Map;

/**
 * <p>
 * ERP仓库组织编码 服务类
 * </p>
 *
 * @author gal
 * @since 2021-12-07
 */
public interface IErpWarehouseCodeService extends IService<ErpWarehouseCode> {

  String getOrganizationCode(String param);

  ResponseData getOrganizationCodeByWarehouse(String param);

  String getSkuSalesOrganization(ErpWarehouseCodeParam param );

  /**
   * 拿到OrderId去结算报告里匹配销售组织
   * @param merchantOrderId
   * @return
   */
  String getOtherSalesOrganization(String merchantOrderId );

  List<Map<String,String>> orgCodeMap(ErpWarehouseCodeParam param);

  String getSalesOrganization(String orderId );

  /**
   * 获取AmazonERP仓库组织信息
   * @return
   */
  List<Map<String,String>> getOrgCodeInfo();

  /**
   * 获取AmazonERP组织名称对应的组织编码
   * @return
   */
  Map<String,String> getOrgCodeMap();

  Map<String,String> getOrgMap();
}
