...
*/
@RestController
@ApiResource(name = "推样预案", path = "/pushSamplePlan")
@Api(tags = "推样预案")
public class PushSamplePlanController {

    @Autowired
    private IRdPreProposalService rdPreProposalService;

    @PostResource(name = "推样预案-列表查询", path = "/listPage", menuFlag = true)
    @ApiOperation(value = "推样预案-列表查询")
    @BusinessLog(title = "推样预案-列表查询",opType = LogAnnotionOpTypeEnum.QUERY)
    public ResponseData listPage(@RequestBody RdPreProposalParam param) {
        param.setSysYaScene(RdPreProposalEnum.YA_SCENE_PUS.getName());
        return ResponseData.success(this.rdPreProposalService.listPage(param));
    }

    @PostResource(name = "推样预案-创建/编辑预案", path = "/addOrEdit")
    @ApiOperation(value = "推样预案-创建/编辑预案")
    @BusinessLog(title = "推样预案-创建/编辑预案",opType = LogAnnotionOpTypeEnum.ADD)
    public ResponseData addOrEdit(@RequestBody RdPreProposalParam param) {
        param.setSysYaScene(RdPreProposalEnum.YA_SCENE_PUS.getName());
        return this.rdPreProposalService.addOrEdit(param);
    }

    @PostResource(name = "推样预案-详情", path = "/detail")
    @ApiOperation(value = "推样预案-详情")
    @BusinessLog(title = "推样预案-详情",opType = LogAnnotionOpTypeEnum.DETAIL)
    ...
    @PostResource(name = "推样预案-申请", path = "/apply")
    @ApiOperation(value = "推样预案-申请")
    @BusinessLog(title = "推样预案-申请",opType = LogAnnotionOpTypeEnum.UPDATE)
    public ResponseData apply(@RequestBody RdPreProposalParam param) {
        param.setSysYaScene(RdPreProposalEnum.YA_SCENE_PUS.getName());
        return this.rdPreProposalService.apply(param);
    }

    @PostResource(name = "推样预案-归档", path = "/archive")
    @ApiOperation(value = "推样预案-归档")
    @BusinessLog(title = "推样预案-归档",opType = LogAnnotionOpTypeEnum.UPDATE)
    public ResponseData archive(@RequestBody RdPreProposalParam param) {
        return this.rdPreProposalService.archive(param);
    }

    @PostResource(name = "推样预案-撤销", path = "/revoke")
    @ApiOperation(value = "推样预案-撤销")
    @BusinessLog(title = "推样预案-撤销",opType = LogAnnotionOpTypeEnum.UPDATE)
